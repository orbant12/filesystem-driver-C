...
def run_cmd(args, allow_err=False, timer=TIMEOUT):
...
def randpath(depth=0, minpartlen=3, maxpartlen=6, is_dir=False, avoid=None):
    ...
    while True:
        ...
        if is_dir:
            path = path + '/'
        for subpath in path_iter(path):
            if subpath.strip('/') in avoid:
                break
        else:
            return path
...
def generate_random_contents(num_files=5, num_dirs=3, max_depth=3, avoid=None,
    ...
    for _ in range(num_files):
        ...
        fname = os.path.join(prefix_dir,
               randpath(depth=depth, avoid=avoid + prev).lstrip('/'))
    ...
    for _ in range(num_dirs):
        ...
        dname = os.path.join(prefix_dir,
                randpath(depth=depth, is_dir=True, avoid=avoid + prev).lstrip('/'))
        conts.append(dname)
        prev.append(dname)
...
class Filesystem:
    ...
    def check_rmdir(self, path):
        ...
        # Apparently "rm -d" does not return same error as os.rmdir
#        retcode, out, err = run_cmd(["rm", "-d", hostpath], timer=5, allow_err=True)
#        if retcode and err.find("Directory not empty") != -1:
#            raise OSError(errno.ENOTEMPTY, f"{err}")
#        if retcode and err.find("No such file or directory") != -1:
